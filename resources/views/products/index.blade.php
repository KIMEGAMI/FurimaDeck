<x-app-layout>
    ...
    <div class="min-h-screen bg-slate-100 py-8"><div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <form class="mb-6 grid gap-3 rounded-lg bg-white p-4 shadow md:grid-cols-6">
            <input name="keyword" value="{{ $filters['keyword'] ?? '' }}" placeholder="商品ID・商品名・型番" class="rounded border-slate-300 text-slate-900 md:col-span-2">
        ...
        <div class="overflow-hidden rounded-lg bg-white shadow"><table class="w-full text-left text-sm text-slate-800"><thead class="bg-slate-100 text-xs"><tr><th class="p-3">画像</th><th class="p-3">商品ID</th><th class="p-3">商品</th><th class="p-3">在庫</th><th class="p-3">仕入原価</th><th class="p-3">状態</th><th class="p-3"></th></tr></thead><tbody>
        @forelse ($products as $product)<tr class="border-t"><td class="p-3"></td><td class="p-3 font-mono">{{ $product->internal_sku }}</td><td class="p-3"><p class="font-bold">{{ $product->product_name }}</p><p class="text-xs text-slate-500">{{ $product->category?->name ?? '未分類' }}</p></td><td class="p-3">{{ number_format($product->quantity_available) }}</td><td class="p-3">¥{{ number_format($product->purchase_unit_cost) }}</td><td class="p-3">{{ $product->isSoldOutByValidSale() ? '売却済み' : \App\\Models\\Product::inventoryStatusLabel($product->inventory_status) }}</td><td class="p-3"><a class="font-bold text-cyan-700" href="{{ route('products.edit', $product) }}">編集</a></td></tr>@empty<tr><td colspan="7" class="p-8 text-center text-slate-500">商品はまだありません。</td></tr>@endforelse
